...
*/
int32_t ALACDecoderPs::Init( void * inMagicCookie, uint32_t inMagicCookieSize )
{
    ...
    if (theCookieBytesRemaining >= sizeof(ALACSpecificConfig))
    {
        ...
        if (!mMixBufferU) mMixBufferU = (int32_t *) calloc( mConfig.frameLength * sizeof(int32_t), 1 );
        mMixBufferV = (int32_t *) heap_caps_calloc( mConfig.frameLength * sizeof(int32_t), 1, MALLOC_CAP_SPIRAM );
        if (!mMixBufferV) mMixBufferV = (int32_t *) calloc( mConfig.frameLength * sizeof(int32_t), 1 );
        ...
        if (!mPredictor) mPredictor = (int32_t *) calloc( mConfig.frameLength * sizeof(int32_t), 1 );
...
/*
	...
	- the decoded samples are interleaved into the output buffer in the order they arrive in
	  the bitstream
*/
int32_t ALACDecoderPs::Decode( BitBuffer * bits, uint8_t * sampleBuffer, uint32_t numSamples, uint32_t numChannels, uint32_t * outNumSamples )
{
	BitBuffer			shiftBits;
...
*/
int32_t ALACDecoderPs::FillElement( BitBuffer * bits )
{
	int16_t		count;
...
*/
int32_t ALACDecoderPs::DataStreamElement( BitBuffer * bits )
{
