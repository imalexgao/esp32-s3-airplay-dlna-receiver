...
void app_main(void) {
  ...
  /* The esp-wifi driver logs Block-Ack (ADDBA) setup retries at INFO when a
   * client connects to the AP (e.g. a phone joining esp32-airplay2); when the
   * negotiation stalls the driver retries in a tight loop, flooding the UART
   * console (~10 kByte/s of identical "<ba-add>" lines that drown the real
   * logs). This is driver chatter, not a fault — AirPlay UDP audio does not
   * need aggregation. Drop the whole wifi module to WARN. */
