...
#include "audio_output.h" /* device volume (USB host output stage) */
#include "dac.h"
...
/* Apply the DEVICE volume (the web slider / hardware buttons control this,
 * NOT the phone volume). USB-host builds apply it at the output stage on top
 * of the AirPlay volume; DAC builds push it to the I2S DAC chip. */
static void apply_device_volume(float db) {
...
static void airplay_adjust_volume(float step_db) {
  ...
  if (s_muted) {
    ...
    apply_device_volume(new_db);
  }
  settings_set_volume(new_db);

  ESP_LOGI(TAG, "Device volume: %.1f -> %.1f dB%s", current_db, new_db,
           s_muted ? " (muted)" : "");
}
...
/* Absolute-volume setter used by the web UI (same path as the volume buttons):
 * sets the independent DEVICE volume (persisted in NVS). It deliberately does
 * NOT touch the AirPlay/source volume and does NOT send DACP to the phone —
 * the web slider controls the speaker, not the phone's slider. */
void playback_control_set_volume(float volume_db) {
  float new_db = clamp_volume(volume_db);
  if (s_muted) {
    /* Update saved level so unmute restores the new volume */
    s_pre_mute_db = new_db;
  } else {
  ...
  ESP_LOGI(TAG, "Device volume set via web: %.1f dB", new_db);
}
